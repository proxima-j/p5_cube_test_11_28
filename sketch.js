...
const slices = 14;
const numShapes = 700;
var isAnimating = false; // Added this line
...
function setup() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    button.position(i * width / 4, 0);
    button.size(width / 4, height);

    // Set initial opacity to 1.0 (fully opaque)
    button.style('transition', 'opacity 0.05s');
    button.style('opacity', '1.0');

    // Use a variable to keep track of the button's visibility state
    let isVisible = true;
